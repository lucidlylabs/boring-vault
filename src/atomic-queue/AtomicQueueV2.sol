...
contract AtomicQueueV2 is Auth, ReentrancyGuard, IPausable {
    ...
    error BoringOnChainQueue__CannotCoverDeficit(uint256);
    ...
    function onchainWithdrawAtomic(address assetOut, uint128 amountOfShares, address teller)
        external
        virtual
        requiresAuth
        returns (bytes32 requestId)
    {
        ...
        // setting discount and secondsToDeadline as 0
        _beforeNewRequest(withdrawAsset, amountOfShares, 0, 0);

        boringVault.safeTransferFrom(msg.sender, address(this), amountOfShares);

        OnChainWithdraw[] memory requests = new OnChainWithdraw[](1);

        (requestId, requests[0]) =
            _queueOnChainWithdraw(msg.sender, assetOut, amountOfShares, 0, withdrawAsset.secondsToMaturity, 0);
        ...
        uint256 assetsOut = TellerWithMultiAssetSupport(teller).bulkWithdraw(solveAsset, totalShares, 0, address(this));

        if (assetsOut > requiredAssets) {
            // transfer excess assets to boring vault
            solveAsset.safeTransfer(address(boringVault), assetsOut - requiredAssets);
        } else if (assetsOut > requiredAssets) {
            revert BoringOnChainQueue__CannotCoverDeficit(requiredAssets - assetsOut);
        }

        solveAsset.safeTransfer(requests[0].user, requests[0].amountOfAssets);
    }
